...
docker run --rm --net drupal-cachebackend_default amazeeio/dockerize dockerize -wait tcp://mariadb:3306 -timeout 1m
```
...
docker ps --filter label=com.docker.compose.project=drupal-cachebackend | grep Up | grep drupal-cachebackend-nginx-1
docker ps --filter label=com.docker.compose.project=drupal-cachebackend | grep Up | grep drupal-cachebackend-mariadb-1
docker ps --filter label=com.docker.compose.project=drupal-cachebackend | grep Up | grep drupal-cachebackend-php-1
docker ps --filter label=com.docker.compose.project=drupal-cachebackend | grep Up | grep drupal-cachebackend-cli-1

# Should ssh against the cli container by default
docker compose exec -T cli bash -c "env | grep LAGOON=" | grep cli-drupal

# Should have the correct environment set
docker compose exec -T cli bash -c "env" | grep LAGOON_ROUTE | grep drupal-cachebackend.docker.amazee.io
